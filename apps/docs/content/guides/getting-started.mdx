...
Before you begin, make sure the following tools are installed and meet the version thresholds. Run `piltover doctor` at any time to check. See [Doctor](/docs/repo/doctor) for the full table.
...
The most common first contribution is adding a Kody rule — a lightweight markdown file that teaches the Kodus PR reviewer about a convention in this codebase. See [Adding a Kody rule](/docs/guides/adding-a-rule) for a step-by-step walkthrough.
